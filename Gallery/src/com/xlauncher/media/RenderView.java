...
//                    paddedWidth = paddedWidth < 128 ? 128 :paddedWidth;
//                    paddedHeight = paddedHeight < 128 ? 128 :paddedHeight;
                    ...
                    config = Bitmap.Config.ARGB_8888;
                    Bitmap padded;
					if (texture.mNeedScaleBmp) {
						padded = Bitmap.createScaledBitmap(bitmap, paddedWidth,
								paddedHeight, true);
					} else {
						padded = Bitmap.createBitmap(paddedWidth, paddedHeight,
								config);
                    ...
                    // coordinates.
                    texture.mNormalizedWidth = 1.0f;//(float) width / (float) paddedWidth;
                    texture.mNormalizedHeight = 1.0f;//(float) height / (float) paddedHeight;
                } else {
    ...
    public void onSurfaceChanged(GL10 gl1, int width, int height) {
        ...
        gl.glLoadIdentity();
        GLU.gluPerspective(gl, 10.5f, (float) width / height, zNear, zFar);
        if (mRootLayer != null) {
