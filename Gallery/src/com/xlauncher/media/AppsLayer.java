...
public class AppsLayer extends RootLayer implements MediaFeed.Listener, TimeBar.Listener {
    ...
    final private AppsInputProcessor mInputProcessor;
    ...
    private final AppsBackgroundLayer mBackground;
	...
	public AppsLayer(Context context, int itemWidth, int itemHeight, LayoutInterface layoutInterface, RenderView view) {
		// TODO Auto-generated constructor stub
        mBackground = new AppsBackgroundLayer(this);
        mContext = context;
        mView = view;
...
//	        }
	    ...
	    public synchronized void onFeedChanged(MediaFeed feed, boolean needsLayout) {
	        if (!needsLayout && !mFeedAboutToChange) {
	        	Log.d(TAG, "onFeedChanged NOT LAYOUT");
	            mFeedChanged = true;
	            forceRecomputeVisibleRange();
	        ...
	        Log.d(TAG, "onFeedChanged  LAYOUT");
	    ...
	    public boolean tapGesture(int slotIndex, boolean metadata) {
	    	Log.d(TAG, "tapGesture slotIndex = " + slotIndex);
	        MediaFeed feed = mMediaFeed;
	        if (feed == null)
	            return false;
	        if (!feed.isClustered()) {
